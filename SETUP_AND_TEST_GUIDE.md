...
- `GOOGLE_API_KEY` for real Gemini chat responses.
- `GOOGLE_APPLICATION_CREDENTIALS` only if using a local service account key for GCP services.

Notes:
- If `GOOGLE_CLOUD_PROJECT` is not set, local env/.env fallback is used.
- If `GOOGLE_API_KEY` is not set, orchestrator falls back to mock LLM behavior.
...
3. Validate mock LLM fallback (no API key):
...
```powershell
$body = @{ message = "hello"; thread_id = "smoke-fallback" } | ConvertTo-Json
Invoke-RestMethod -Uri http://localhost:8000/chat -Method Post -ContentType "application/json" -Body $body
```

Expected: response starts with `[Mock] Gemini API not configured...`.

4. Validate real LLM path (with API key):

```powershell
$env:GOOGLE_API_KEY="your-real-google-api-key"
```

Call chat again:

```powershell
$body = @{ message = "hello"; thread_id = "smoke-real" } | ConvertTo-Json
Invoke-RestMethod -Uri http://localhost:8000/chat -Method Post -ContentType "application/json" -Body $body
```

Expected: response is generated by Gemini (not mock-prefixed).
...
3. Mock LLM warning (`No GOOGLE_API_KEY found`):
- Expected if `GOOGLE_API_KEY` is missing.
- Set `GOOGLE_API_KEY` to enable real Gemini responses.
