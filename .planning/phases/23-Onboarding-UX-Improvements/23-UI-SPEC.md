...
- **Choice Poll**: After trigger, present a `Choice Poll` to capture user intent:
    - **Questions**: "What is your primary focus?" (Backend, Frontend, Fullstack, Just Exploring).
    - **CTA**: "Personalize My Guide" (Icon: `ArrowRight`).
    - **Impact**: Poll results are sent to the backend to customize the generated plan.
...
- **Accessibility**: 
    - Container marked with `aria-busy="true"`.
    - Status list uses `aria-live="polite"` for automatic updates.
- **Feedback**:
...
- **Body**: `var(--text-body)` (15px), Regular, `1.6` line-height.
...
| `SetupGuide` | New | Block with copyable shell commands and "Copy Command" button. |
...
- **Poll Submit**: "Personalize My Guide"
...
|----------|-------|---------------|----------|
| `cult-ui` | `onboarding` | `view passed` | Standard motion-based stepper, vetted 2026-05-10. |
| `cult-ui` | `intro-disclosure`| `view passed` | Clean disclosure pattern, vetted 2026-05-10. |
| `cult-ui` | `choice-poll` | `view passed` | Interactive variant, vetted 2026-05-10. |
